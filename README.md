...
The `./liquefy` wrapper automatically pulls and runs the sealed decoder container:
`parad0xlabs/liquefy-decoder-public:latest`
...
- **Sealed AF:** Compression, orchestration, and engine tuning remain proprietary and are not shipped in public repos.
...
2.  **Execution Safety:** To prevent runtime instability, the appliance requires a standard execution environment. If unauthorized runtime hooks (e.g. `LD_PRELOAD`) are detected, the appliance will fail closed (Code 13).
