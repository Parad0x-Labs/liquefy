#!/usr/bin/env python3
"""
NULL_Apache_Repetition_Focused - [NULL REPETITION v1]
=====================================================
TARGET: 7000x+ Compression on highly repetitive Apache logs.
...
class NULL_Apache_Repetition_Focused:
    ...
    def compress(self, raw: bytes) -> bytes:
        ...
        # Use regex to find Apache fields while preserving static parts
        # Format: IP - - [TS] "REQ" CODE SIZE "REF" "UA"
        import re
        ap_re = re.compile(rb'^(\S+) - - \[(.*?)\] "(.*?)" (\d+|-) (\d+|-) "(.*?)" "(.*?)"(\r?\n?)$')
        ...
        for line in lines:
            m = ap_re.match(line)
            if m:
                ip, ts, req, code_b, size_b, ref, ua, eol = m.groups()
                ...
                pat_key = (ip, ts, req, ref, ua, eol) # Include EOL in pattern
        ...
        for (ip, ts, req, ref, ua, eol) in pat_list:
            ...
            for col, val in [(col_ts, ts), (col_req, req), (col_ref, ref), (col_ua, ua), (col_eol, eol)]:
                col.extend(pack_varint(len(val))); col.extend(val)
        
        payload = bytearray()
        def add_chunk(b): payload.extend(pack_varint(len(b))); payload.extend(b)
        for c in [col_ip, col_ts, col_req, col_ref, col_ua, col_eol, s_pats, s_code, s_size, s_raw]: add_chunk(c)
    ...
    def decompress(self, blob: bytes) -> bytes:
        ...
        p = 0
        chunks = []
        for _ in range(10): chunk, p = get_chunk(p); chunks.append(chunk)
        
        c_ip, c_ts, c_req, c_ref, c_ua, c_eol, s_pats, s_code, s_size, s_raw = chunks
        pat_list = []; p_ip = 0; p_ts = 0; p_req = 0; p_ref = 0; p_ua = 0; p_eol = 0
        while p_ip < len(c_ip):
            ...
            ts, p_ts = next_v(c_ts, p_ts); req, p_req = next_v(c_req, p_req)
            ref, p_ref = next_v(c_ref, p_ref); ua, p_ua = next_v(c_ua, p_ua); eol, p_eol = next_v(c_eol, p_eol)
            pat_list.append((ip, ts, req, ref, ua, eol))
        ...
        while p_pats < len(s_pats):
            ...
            if pid == 0:
                ...
                for _ in range(count):
                    ...
                    out.extend(pat[0] + b' - - [' + pat[1] + b'] "' + pat[2] + b'" ' + c_str + b' ' + s_str + b' "' + pat[3] + b'" "' + pat[4] + b'"' + pat[5])
        return bytes(out)
...
if __name__ == "__main__":
    if len(sys.argv) < 3: print("Usage: python NULL_Apache_Repetition_Focused.py [compress|decompress|grep] <in> <out/query>"); sys.exit(1)
    codec = NULL_Apache_Repetition_Focused()
